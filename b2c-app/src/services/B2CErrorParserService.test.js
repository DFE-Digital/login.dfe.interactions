...
describe('when parsing an error coming from B2C', () => {
    ...
    it('does return a paragraph with the passed in error if not in transformation list', async () => {
        let testError = 'test_error';
        expect(B2CErrorParserService.parseB2CErrors(testError)).toEqual(<p>{testError}</p>);
    });

    it('does return the configured transformed error message when it matches one', async () => {
